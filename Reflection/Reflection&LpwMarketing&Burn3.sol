...
contract Token is Context, IERC20, Ownable {
    ...
    function setMaxBuyPercent(uint256 _mtxp, bool _mtx) external onlyOwner() {require(_mtxp >= 1); _maxBuyAmount = _tTotal * (_mtxp) / (10**3); maxTxEnabled = _mtx;}
    ...
    function checkLaunched(address from) internal view {
            require(wenLaunch.Launched || isExcludedFromFee(from), "Pre-Launch Protection");
        }           
    ...
    function _transfer(
        ...
        checkLaunched(from);
        if(!wenLaunch.Launched && to == uniswapV2Pair && _isExcludedFromFee[from]){require(_tOwned[from] > 0); launch(); }
        if(maxTxEnabled){checkBuy(from, to, amount); checkSell(from, amount);}    
        if(shouldSwapBack()) {swapAndLiquify(numTokensSellToAddToLiquidity);}
