...
contract Token is Context, IERC20, Ownable {
    ...
    uint256 _previousBuyLiquidityFee = _buyliquidityFee;
    uint256 _previousBuyTaxFee = _buytaxFee;
    uint256 _previousBuyBurnFee = _buyburnFee;
    ...
    uint256 maxTaxFee = 5;
    uint256 maxLiquidityFee = 5;
    uint256 maxBurnFee = 5;
    ...
    bool buycooldownEnabled = true;
    bool sellcooldownEnabled = true; 
    uint256 _cooldown = 30 seconds;
    uint256 constant _cooldownCap = 60 seconds;

    uint256 maxTxPercent = 1;
    uint256 maxTxDivisor = 100;
    uint256 public _maxBuyAmount = (_tTotal * maxTxPercent) / maxTxDivisor;
    uint256 public _maxSellAmount = _maxBuyAmount / 2;
    ...
    function setCooldownEnabled(bool onoff, bool truefalse) public onlyOwner() {buycooldownEnabled = onoff;sellcooldownEnabled = truefalse;}
    function cooldownStatus() public view returns (bool, bool, uint256, uint256) {return (buycooldownEnabled, sellcooldownEnabled, _cooldown, _cooldownCap);}    
    function setCooldown(uint256 cooldown) public onlyOwner() {require(cooldown <= _cooldownCap); _cooldown = cooldown;}
    function setSwapInteral(uint256 interval) public onlyOwner() {swapInterval = interval;}
    function setMaxBuyPercent(uint256 _mtxp) external onlyOwner() {require(_mtxp >= 1); _maxBuyAmount = _tTotal * (_mtxp) / (10**3);}
    ...
    function viewBuyFees() public view returns (uint256, uint256, uint256) {return(_buytaxFee, _buyburnFee, _buyliquidityFee);}
    function viewFees() public view returns (uint256, uint256, uint256) {return(_taxFee, _burnFee, _liquidityFee);}
    ...
    function clearStuckTokens(address _token, address _to) external onlyOwner returns (bool _sent){require(_token != address(0));uint256 _contractBalance = IERC20(_token).balanceOf(address(this));_sent = IERC20(_token).transfer(_to, _contractBalance);}
    ...
    function setFees(uint256 taxFee, uint256 liquidityFee, uint256 burnFee) external onlyOwner() {require(taxFee <= maxTaxFee && liquidityFee <= maxLiquidityFee && burnFee <= maxBurnFee);require(taxFee + liquidityFee + burnFee <= 1500);_taxFee = taxFee;_liquidityFee = liquidityFee;_burnFee = burnFee;}
    function setBuyFees(uint256 taxFee, uint256 liquidityFee, uint256 burnFee) external onlyOwner() {require(taxFee <= maxTaxFee && liquidityFee <= maxLiquidityFee && burnFee <= maxBurnFee);require(taxFee + liquidityFee + burnFee <= 1500);_buytaxFee = taxFee;_buyliquidityFee = liquidityFee;_buyburnFee = burnFee;}
    function _reflectFee(uint256 rFee, uint256 tFee) private {_rTotal = _rTotal -= (rFee);_tFeeTotal = _tFeeTotal += (tFee);}
    function restoreAllFee() private {_taxFee = _previousTaxFee;_liquidityFee = _previousLiquidityFee;_burnFee = _previousBurnFee;_buyburnFee = _previousBuyBurnFee;_buyliquidityFee = _previousBuyLiquidityFee;_buytaxFee = _previousBuyTaxFee;}
    function removeAllFee() private {if(_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _buytaxFee == 0 && _buyliquidityFee == 0 && _buyburnFee == 0) return;_previousTaxFee = _taxFee;_previousLiquidityFee = _liquidityFee;_previousBurnFee = _burnFee;_previousBuyBurnFee = _buyburnFee;_previousBuyTaxFee = _buytaxFee;_previousBuyLiquidityFee = _buyliquidityFee;_buyliquidityFee = 0;_buyburnFee = 0;_buytaxFee = 0;_burnFee = 0;_taxFee = 0;_liquidityFee = 0;}
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tBurn = (tAmount * _burnFee) / 10**2;
    ...
    function _getBuyTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tFee = (tAmount * _buytaxFee) / 10**2;
        uint256 tLiquidity = (tAmount * _buyliquidityFee) / 10**2;
        uint256 tBurn = (tAmount * _buyburnFee) / 10**2;
        uint256 tTransferAmount = tAmount -= (tFee + tLiquidity);
        return (tTransferAmount, tFee, tLiquidity, tBurn);
    }
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to] && buycooldownEnabled) {
                ...
                buycooldown[to] = block.timestamp + (_cooldown);
        } else if (from != uniswapV2Pair && sellcooldownEnabled && !_isExcludedFromFee[from]) {
            require(amount <= _maxSellAmount, "Transfer amount exceeds the MaxSellAmount");
            require(sellcooldown[from] <= block.timestamp);
            sellcooldown[from] = block.timestamp + (_cooldown);
        }
       if(swapAndLiquifyEnabled) {
        uint256 contractTokenBalance = balanceOf(address(this));
        bool overMinTokenBalance = contractTokenBalance >= numTokensSellToAddToLiquidity;
        bool overMinSwapInterval = block.timestamp >= lastSwap + swapInterval;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            from != uniswapV2Pair &&
