...
contract T is Context, IERC20, Ownable {
    ...
    uint256 private _previousTaxFee = Fees._taxFee;
    uint256 private _previousLiquidityFee = Fees._liquidityFee;
    uint256 private _previousBurnFee = Fees._burnFee;
    ...
    uint256 public _maxTxAmount = _tTotal / 10**2;
    ...
    constructor (address payable m) {
        ...
        _isExcludedFromFee[tokenAddress] = true;
    ...
    function clearStuckBalance(uint256 amountPercentage) public onlyOwner {
        ...
        uint256 amountBNB = tokenAddress.balance;
        payable(_marketingWallet).transfer(amountBNB * amountPercentage / 100);
    }

    function clearStuckTokens(address _token, address _to) public onlyOwner returns (bool _sent){
        require(_token != address(0));
        uint256 _contractBalance = IERC20(_token).balanceOf(tokenAddress);
    ...
    function setFees(uint256 liquidityFee, uint256 taxFee, uint256 burnFee) external onlyOwner() {
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        uint256 currentRate =  _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        _rOwned[tokenAddress] = _rOwned[tokenAddress].add(rLiquidity);
        if(_isExcluded[tokenAddress])
            _tOwned[tokenAddress] = _tOwned[tokenAddress].add(tLiquidity);
    }
    ...
    function removeAllFee() private {
        if(Fees._taxFee == 0 && Fees._liquidityFee == 0 && Fees._burnFee == 0) return;

        _previousTaxFee = Fees._taxFee;
        _previousLiquidityFee = Fees._liquidityFee;
        _previousBurnFee = Fees._burnFee;
        Fees._taxFee = 0;
        Fees._burnFee = 0;
    ...
    function restoreAllFee() private {
        Fees._taxFee = _previousTaxFee;
        Fees._burnFee = _previousBurnFee;
    ...
    function _transfer(
        ...
        } else if (from != uniswapV2Pair && cooldownInfo.sellcooldownEnabled && !_isExcludedFromFee[from]) { 
            require(sellcooldown[from] <= block.timestamp, "Sell Cooldown not over");
            sellcooldown[from] = block.timestamp + (cooldownInfo._cooldown);}

        uint256 contractTokenBalance = balanceOf(tokenAddress);
    ...
    function manualSwap(uint256 contractBalance, uint256 percent) public onlyOwner{
        require(percent <= 100);
        contractBalance = (balanceOf(tokenAddress) * percent) / 100;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 bnbSwapped = tokenAddress.balance.sub(initialBalance);
