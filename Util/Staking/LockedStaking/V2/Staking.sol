...
contract Staking {
    ...
    function stake(uint256 amount, uint256 planIndex) public {
        ...
        stakersRecord[msg.sender][index].persecondreward = stakersRecord[msg.sender][index].reward / Duration[planIndex];
        stakersRecord[msg.sender][index].plan = planIndex;
        Stakers[msg.sender].stakeCount++;
        userStakedPerPlan[msg.sender][planIndex] += amount;
        totalStakedPerPlan[planIndex] += amount;
