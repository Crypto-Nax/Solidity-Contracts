...
contract NftStaking is ERC721Holder, ReentrancyGuard, Ownable, Pausable {
    using Address for address;
    ...
    // uint256 endTime;
    uint256 totalSupply;
    uint256 rewardCycle = 24 hours;

    mapping(address => UserInfo) userInfo;
    
    EnumerableSet.UintSet totalNfts;

    IERC721A StakingNftToken;
    IERC20 rewardToken;
    ...
    modifier updateRewards(){
        ...
        uint multiplier = getMultiplier(lastUpdateTime, block.number);
        uint256 reward = multiplier.mul(rewardRate);
        accPerShare = accPerShare.add(reward);
        lastUpdateTime = block.number;

        user.pendingRewards = user.pendingRewards.add(user.amount.mul(accPerShare).sub(user.rewardDebt));
        _;
        
        user.rewardDebt = user.amount.mul(accPerShare);
        user.claimedAt = block.timestamp;
    }

    constructor(IERC20 Token, IERC721A Nft) {
        StakingNftToken = Nft;
        rewardToken = Token;
    }
    ...
    function claimRewards() internal nonReentrant updateRewards {
        ...
        if (user.pendingRewards > 0 && block.timestamp.sub(user.claimedAt) >= rewardCycle && totalSupply > 0) {
            user.pendingRewards = 0;         
            rewardToken.safeTransfer(_msgSender(), pending);
            user.rewardsDistributed += pending;
            totalSupply -= pending;
            emit Claimed(_msgSender(), pending);
        }
    }

    function update() internal updateRewards{
    ...
    function setRewardRate(uint256 _rewardRate) external {
        ...
        uint multiplier = getMultiplier(lastUpdateTime, block.number);
        uint256 reward = multiplier.mul(rewardRate);
        accPerShare = accPerShare.add(reward.div(totalSupply));
        lastUpdateTime = block.number;
    ...
    function claimable() external view returns (uint256) {
        ...
        uint multiplier = getMultiplier(lastUpdateTime, block.number);
        uint256 reward = multiplier.mul(rewardRate);
        curAccPerShare = accPerShare.add(reward);
        return user.pendingRewards.add(user.amount.mul(curAccPerShare).sub(user.rewardDebt));
    }
    ...
    function depositNft(uint256 NftId) external whenNotPaused {
        ...
        totalNfts.add(NftId);
        NFTs.amount = NFTs.nftIds.length();
    ...
    function batchDepositNft(uint256[] memory NftId) external whenNotPaused {
        ...
        for(uint256 i; i < NftId.length; i++){
            if(StakingNftToken.ownerOf(NftId[i]) == _msgSender()){
                StakingNftToken.safeTransferFrom(_msgSender(), address(this), NftId[i]);
                if(NFTs.nftIds.length() == 0) stakers++;
                NFTs.nftIds.add(NftId[i]);
                totalNfts.add(NftId[i]);
            }
        }
        NFTs.amount = NFTs.nftIds.length();
    ...
    function batchWithdrawNft(uint256[] memory NftId) external {
        ...
        for(uint256 i; i < NftId.length; i++){
            if(NFTs.nftIds.contains(NftId[i])){
                StakingNftToken.safeTransferFrom(address(this), _msgSender(), NftId[i]);
                NFTs.nftIds.remove(NftId[i]);
                totalNfts.remove(NftId[i]);
            }
        }
        NFTs.amount = NFTs.nftIds.length();
        if(NFTs.nftIds.length() == 0) stakers--;
        emit NftBatchWithdrawl(_msgSender(), NftId);
        update();
    }

    function getStakingDetails() external view returns(address nftAddress, address tokenAddress, uint256 _lastUpdateTime, uint256 _rewardRate, uint256 _accPerShare, uint256 _totalSupply, uint256 _rewardCycle, uint256 _stakers) {
        nftAddress = address(StakingNftToken);
        tokenAddress = address(rewardToken);
        _lastUpdateTime = lastUpdateTime;
        _rewardRate = rewardRate;
        _accPerShare = accPerShare;
        _totalSupply = totalSupply;
        _rewardCycle = rewardCycle;
        _stakers = stakers;
    }

    function getUserDetails() external view returns(uint256 _claimedAt, uint256 _rewardsDistributed, uint nftAmount){
        UserInfo storage user = userInfo[_msgSender()];
        _rewardsDistributed = user.rewardsDistributed;
        _claimedAt = user.claimedAt;
        nftAmount = user.amount;
    }

    function usersNfts() external view returns (uint256[] memory, uint256 nftAmount) {
