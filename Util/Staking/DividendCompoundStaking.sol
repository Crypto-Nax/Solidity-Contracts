...
contract Staking is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public penaltyFee = 500;
    uint256 public constant MAX_FEE = 10000;
    uint256 public constant FEE_LIMIT = 500; // 5%
    uint256 public rewardRate = uint256(0.00001 gwei);
    ...
    mapping(address => AutoCompounded) public compoundInfo;
    ...
    constructor(IERC20 _stakingToken, IERC20 _dividendToken) {
        stakingToken = _stakingToken;
        rewardToken = _stakingToken;
        dividendToken = _dividendToken;
        feeRecipient = msg.sender;
        lastUpdateTime = block.timestamp;
        endTime = block.timestamp.add(365 days / 2); // In default, half a year
    }
    ...
    function setCompound() public {
        AutoCompounded storage compounding = compoundInfo[msg.sender];
        if(!compoundInfo[msg.sender].autoCompound) {
            compounding.autoCompound = true;
            _checkOrAddCompounder(msg.sender);
        } else {
            compounding.autoCompound = false;
            _removeCompounder(msg.sender);
        }
    }

    function compoundGains(uint256 index) internal {
        UserInfo storage user = userInfo[compoundList(index)];
        AutoCompounded storage compounder = compoundInfo[compoundList(index)];
        uint256 compoundedAmount = user.pendingRewards;
        _safeTransferDividends(compoundList(index), compoundedAmount);
        totalReward = totalReward.sub(compoundedAmount);
        compounder.claimedAt = block.timestamp;
    ...
    function processGains() internal {
        if(compoundCount() == 0){return;}
        uint256 gasUsed;
        uint256 gasLeft = gasleft();
        uint256 iterations;
        uint256 count;
        while(gasUsed < compoundGas && iterations < compoundCount()){
            if(currentIndex >= compoundCount()) {
                currentIndex = 0;
            }
            if(shouldAutoCompound(currentIndex)) {
                compoundGains(currentIndex);
                count++;
            }
    ...
    function withdrawPercent(uint256 percent) external {
        UserInfo storage user = userInfo[msg.sender];
        require(percent <= 100);
    ...
    function claim() public nonReentrant updateReward updateUserList {
        AutoCompounded storage compounder = compoundInfo[msg.sender];
        UserInfo storage user = userInfo[msg.sender];
        if(autoCompound.contains(msg.sender)){
            require(block.timestamp.sub(compounder.claimedAt) >= rewardCycle, "!available still");
            uint256 compoundedAmount = user.pendingRewards;
            _safeTransferDividends(msg.sender, compoundedAmount);
            totalReward = totalReward.sub(compoundedAmount);
            compounder.claimedAt = block.timestamp;
            user.pendingRewards.sub(compoundedAmount);
            user.claimedAt = block.timestamp;
            user.amount += compoundedAmount;          
    ...
    function shouldAutoCompound(uint256 index) internal view returns(bool cycleDone) {
        return block.timestamp - compoundInfo[compoundList(index)].claimedAt >= rewardCycle;
    }
    ...
    function compoundList(uint256 index) public view returns (address indexedAddress) {
