...
contract RewardToken is Pausable, AccessControl, ERC20Permit {
    ...
    IRouter02 immutable _initialRouter;        
    DividendDistributor public _distributor;
    address immutable _initialPair;
    ...
    function setMarketingFeeReceiver(address receiver) public onlyRole(ADMIN_ROLE) {
        _marketingFeeReceiver = receiver;
        emit FeeReceiversUpdated(receiver, "Marketing");
    }

    function setLiquidityFeeReceiver(address receiver) public onlyRole(ADMIN_ROLE) {
        _autoLiquidityReceiver = receiver;
        emit FeeReceiversUpdated(receiver, "Liquidity");
    }

    function setBuyBackReceiver(address receiver) public onlyRole(ADMIN_ROLE) {
        _buyBackReceiver = receiver;
        emit FeeReceiversUpdated(receiver, "Buyback");
    }
    ...
    function preTransferCheck(address from, address to, uint amount) internal view returns(bool){
        ...
        if(LiquiditySettings.inSwap){
            ...
            if(limits(from, to)){
                ...
                if(Launch.launched){
                    ...
                    uint256 amountReceived = ContractSettings.feesEnabled && !_isFeeExempt[from] ? takeFee(from, to, amount) : amount;
                    ERC20.transfer(to, amountReceived);

                    if (!_isDividendExempt[from] && balanceOf(from) >= holdReq()) { try _distributor.setShare(from, balanceOf(from)) {} catch {} }
                    if (!_isDividendExempt[to] && balanceOf(to) >= holdReq() ) { try _distributor.setShare(to, balanceOf(to)) {} catch {} }
    ...
    function launch(uint8 sniperBlocks) public onlyRole(ADMIN_ROLE) {
        ...
        setMaxFees(400,400,400,400, false);
    ...
    function setSwapBackSettings(bool _enabled, uint256 _amount, bool _liquify) public onlyRole(ADMIN_ROLE){
        ...
        emit SwapBackSettingsUpdated(_enabled, _amount, _liquify);
    }
    ...
    function swapBack() internal swapping() {
        ...
        if (LiquiditySettings.liquidityFeeAccumulator >= LiquiditySettings.numTokensToSwap && LiquiditySettings.autoLiquifyEnabled) {
            ...
            emit SwapBack(amountToLiquify, amountEth);
    ...
    function _manualDeposit(uint256 amount, bool fundReward) external onlyRole(ADMIN_ROLE) {
        ...
        emit ManualDividendDeposit(true, fundReward, amount);
    }
    ...
    function setCooldownExempt(address holder, bool onOff) public onlyRole(ADMIN_ROLE) {
        ...
        emit CooldownExempt(holder, onOff);
    }
    ...
    function setTransactionLimitExempt(address holder, bool setOrRemove) public onlyRole(ADMIN_ROLE){
        ...
        emit TransactionLimitExempt(holder, setOrRemove);
    }
    ...
    // Dividend Functions
    function setIsDividendExempt(address holder, bool exempt) external onlyRole(ADMIN_ROLE){
    ...
    function setBuyFees(uint16 liquidityFee, uint16 reflectionFee, uint16 marketingFee, uint16 buybackFee) public onlyRole(ADMIN_ROLE) {
        ...
        emit FeeSet(liquidityFee, reflectionFee, marketingFee, buybackFee, "Buy Fees");
    }    
    ...
    function setSellFees(uint16 liquidityFee, uint16 reflectionFee, uint16 marketingFee, uint16 buybackFee) public onlyRole(ADMIN_ROLE) {
        ...
        emit FeeSet(liquidityFee, reflectionFee, marketingFee, buybackFee, "Sell Fees");
    ...
    function setTransferFees(uint16 liquidityFee, uint16 reflectionFee, uint16 marketingFee, uint16 buybackFee) public onlyRole(ADMIN_ROLE) {
        ...
        emit FeeSet(liquidityFee, reflectionFee, marketingFee, buybackFee, "Transfer Fees");
    }    
    
    function setMaxFees(uint16 liquidityFee, uint16 reflectionFee, uint16 marketingFee, uint16 buybackFee, bool resetFees) public onlyRole(ADMIN_ROLE) {
        if(!Launch.launched){
            MaxFees = IFees({
                liquidityFee: liquidityFee,
                reflectionFee: reflectionFee,
            ...
            emit FeeSet(liquidityFee, reflectionFee, marketingFee, buybackFee, "Initial Max Fees");
        } else {
            require(liquidityFee <= MaxFees.liquidityFee && reflectionFee <= MaxFees.reflectionFee && marketingFee <= MaxFees.marketingFee && buybackFee <= MaxFees.buyBackFee, "Fee Higher than Max Fees");
            MaxFees = IFees({
                liquidityFee: liquidityFee,
            ...
            emit FeeSet(liquidityFee, reflectionFee, marketingFee, buybackFee, "Max Fees");
            if(resetFees){
                setSellFees(liquidityFee, reflectionFee, marketingFee, buybackFee);
                setBuyFees(liquidityFee, reflectionFee, marketingFee, buybackFee);
            }
        }
    }

    // function shouldTakeFee(address sender) internal view returns (bool) {
    //     return ContractSettings.feesEnabled && !_isFeeExempt[sender];
    // }
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        if(ContractSettings.burnFees){
            ...
            ERC20.transfer(address(this), feeAmount);
        }
        return amount - feeAmount;
    }

    event AutoBuybackSettingsUpdated(bool enabled, uint256 _cap, uint256 _amount, uint256 _period);
    event AutoLiquify(uint256 amountEth, uint256 amountToken);
    event Buyback(uint256 amount);
    event CooldownExempt(address holder, bool onOff);
    ...
    event FeeReceiversUpdated(address indexed receiver, string receiverType);
    event FeeSet(uint16 liquidityFee, uint16 reflectionFee, uint16 marketingFee, uint16 buybackFee, string feeType);   
    event HolderFeeExempt(address holder, bool setOrRemove);
    event Launched();
    event LiquidityHoldersUpdated(address holder, bool setOrRemove);        
    event ManualDividendDeposit(bool manualDeposit, bool fundReward, uint256 manualDepositAmount);
    ...
    event SwapBackSettingsUpdated(bool _enabled, uint256  _amount, bool liquify);
    event TransactionLimitExempt(address holder, bool setOrRemove);
