...
contract Token is IERC20, Context {
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        if (lpPairs[receiver]) {
            ...
            if(SellFees.stakeFee > 0) {
                stakeFee = SellFees.stakeFee;
            }
        } else if(lpPairs[sender]){
            totalFee = BuyFees.totalFee;
            if(BuyFees.stakeFee > 0) {
                stakeFee = BuyFees.stakeFee;
            }
        } else {
            totalFee = TransferFees.totalFee;
            if(TransferFees.stakeFee > 0) {
                stakeFee = TransferFees.stakeFee;
            }
        }

        uint256 feeAmount = (amount * totalFee) / feeDenominator;
        if(stakeFee > 0) {
            uint256 amountForStaking = (feeAmount * (BuyFees.stakeFee + SellFees.stakeFee) / (BuyFees.totalFee + SellFees.totalFee));
            _balances[stakingFeeReceiver] += amountForStaking;
            emit Transfer(sender, address(stakingFeeReceiver), amountForStaking);
            _balances[address(this)] += feeAmount - amountForStaking;
            emit Transfer(sender, address(this), feeAmount - amountForStaking);
            ...
            return amount - (feeAmount + amountForStaking);
        }
        if (LiquiditySettings.autoLiquifyEnabled) {
            LiquiditySettings.liquidityFeeAccumulator += ((feeAmount * (BuyFees.liquidityFee + SellFees.liquidityFee)) / ((BuyFees.totalFee + SellFees.totalFee) + (BuyFees.liquidityFee + SellFees.liquidityFee)));
        }
    ...
    function swapBack() internal swapping {
        ...
        if (LiquiditySettings.liquidityFeeAccumulator >= LiquiditySettings.numTokensToSwap && LiquiditySettings.autoLiquifyEnabled) {
            ...
            if(fundRewards){
                if(manualDeposit) {
                    try distributor.deposit{value: (address(this).balance * 30) /100}() {} catch {}
                    (bool success, ) = payable(marketingFeeReceiver).call{
                    value: amountEthMarketing,
                    gas: 30000}("");
                    if (success) {
                        marketingFees += amountEthMarketing;
                        manualDeposit = false;
                    }
                } else {
                    ...
                    value: amountEthMarketing,
                    gas: 30000}("");
                    if (success) { marketingFees += amountEthMarketing;}  
                }
            } else {
    ...
    function _manualDeposit() external onlyOwner {
        manualDeposit = true;
    }
    ...
    function triggerAutoBuyback() internal {
        buyTokens(BuyBackSettings.autoBuybackAmount, stakingFeeReceiver);
        BuyBackSettings.autoBuybackBlockLast = block.number;
        BuyBackSettings.autoBuybackAccumulator += BuyBackSettings.autoBuybackAmount;
        if (BuyBackSettings.autoBuybackAccumulator > BuyBackSettings.autoBuybackCap) {
            BuyBackSettings.autoBuybackEnabled = false;
        }
    }
    ...
    function setBuyFees(uint16 _liquidityFee, uint16 _buybackFee, uint16 _reflectionFee, uint16 _marketingFee, uint16 _stakeFee) public authorized {
    ...
    function setSellFees(uint16 _liquidityFee, uint16 _buybackFee, uint16 _reflectionFee, uint16 _marketingFee, uint16 _stakeFee) public authorized {
        require(_liquidityFee <= MaxFees.liquidityFee && _reflectionFee <= MaxFees.reflectionFee && _marketingFee <= MaxFees.marketingFee && _buybackFee <= MaxFees.buybackFee && _stakeFee <= MaxFees.stakeFee);
        SellFees = IFees({
            liquidityFee: _liquidityFee,
            buybackFee: _buybackFee,
    ...
    function setTransferFees(uint16 _liquidityFee, uint16 _buybackFee, uint16 _reflectionFee, uint16 _marketingFee, uint16 _stakeFee) public authorized {
        require(_liquidityFee <= MaxFees.liquidityFee && _reflectionFee <= MaxFees.reflectionFee && _marketingFee <= MaxFees.marketingFee && _buybackFee <= MaxFees.buybackFee && _stakeFee <= MaxFees.stakeFee);
        TransferFees = IFees({
            liquidityFee: _liquidityFee,
            buybackFee: _buybackFee,
    ...
    function decreaseMaxFees(uint16 _liquidityFee, uint16 _buybackFee, uint16 _reflectionFee, uint16 _marketingFee, uint16 _stakeFee, bool resetFees) public authorized {
        require(_liquidityFee <= MaxFees.liquidityFee && _reflectionFee <= MaxFees.reflectionFee && _marketingFee <= MaxFees.marketingFee && _buybackFee <= MaxFees.buybackFee && _stakeFee <= MaxFees.stakeFee);
        MaxFees = IFees({
            liquidityFee: _liquidityFee,
            buybackFee: _buybackFee,
   ...
   function setAutoLiquifyEnabled(bool _enabled) public authorized {
    ...
    function setDistributorSettings(uint32 gas, bool _autoClaim)
        external
        authorized
    {
        require(gas <= 1000000);
