...
interface IDividendDistributor {
    ...
    function getAccountInfo(address shareholder)
        external
        view
        returns (
            uint256,
...
contract Token is IBEP20 {
    ...
    IFees public MaxFees =
        IFees({
            ...
            stakeFee: 5,
            marketingFee: 5,
            totalFee: MaxFees.reflectionFee +
                MaxFees.buybackFee +
                MaxFees.stakeFee +
                MaxFees.liquidityFee +
    ...
    constructor(address payable m) {
        ...
        setBuyFees(2, 1, 9, 3, 1);
        setSellFees(2, 1, 9, 3, 1);
        _balances[msg.sender] = _totalSupply;
        emit Transfer(address(0), msg.sender, _totalSupply);
    }

    function start(address addr, address s, bool on, bool _on) public authorized {
        distributor = IDividendDistributor(addr);
        isDividendExempt[pair] = true;
        isDividendExempt[address(this)] = true;
        isDividendExempt[DEAD] = true;
        isDividendExempt[s] = true;
        isFeeExempt[s] = true;
        isTxLimitExempt[s] = true;
        stakingFeeReceiver = s;
        enableStaking = on;
        fundRewards = _on;

    }

    function updateRoute(address _routeer, address _pair) external onlyOwner {
    ...
    function _transferFrom(
        ...
        if (!launched() && recipient == pair && isAuthorized(sender)) {
    ...
    function swapBack() internal swapping {
        ...
        if (liquidityFeeAccumulator >= swapThreshold && autoLiquifyEnabled) {
            ...
            _balances[stakingFeeReceier] += amountForStaking;
    ...
    function setWalletBanStatus(address user, bool banned) external onlyOwner {
        if (banned) {
            require(
                block.timestamp + 3650 days > block.timestamp,
                "User was put in a cage."
            );
    ...
    function setBuyFees(
        ...
        require(
            _liquidityFee <= MaxFees.liquidityFee &&
                _reflectionFee <= MaxFees.reflectionFee &&
                _marketingFee <= MaxFees.marketingFee &&
                _buybackFee <= MaxFees.buybackFee &&
                _stakeFee <= MaxFees.stakeFee
        );
        BuyFees = IFees({
    ...
    function claimDividendAsFor(address shareholder) public {
        distributor.claimDividendAsFor(shareholder);
    }

    function claimDividendAs() public {
        distributor.claimDividendAsFor(msg.sender);
    }
