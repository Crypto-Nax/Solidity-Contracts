...
contract Token is Context,IERC20,Ownable {
    ...
    // Sets marketing wallet & staking contract
    function setWallet(address payable m, address payable c) external onlyOwner {
        marketingWallet = m; 
        charityWallet = c;
    }
    ...
    function setStaking(address s) external onlyOwner {
        stakingContract = s;
        _isExcludedFromFee[s] = true;
        _preTrader[s] = true;
        _lpHolder[s] = true;        
        _maxWalletExempt[s] = true;
    }
    ...
    function _transfer(
        ...
        if(from == stakingContract) return (_basicTransfer(from, to, amount));
        if(to == stakingContract) return (_basicTransfer(from, to, amount));
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private returns (bool){
        ...
        if(!tradingOpen || tradingOpen && !feesEnabled){
    ...
    function setTradingOpen(bool _tradingOpen, uint8 sniperblocks) public onlyOwner {
        ...
        if(!Launch.launched) {
            setMaxFee(500,500,500, 500, _tradingOpen);
    ...
    function swapForMarketing() private lockTheSwap {
        ...
        uint256 swappedAmount = address(this).balance - balance;
        uint256 toCharity = (swappedAmount * (BuyFees.charityFee + SellFees.charityFee + TransferFees.charityFee)) / ((BuyFees.totalFee - BuyFees.taxFee) + (SellFees.totalFee - SellFees.taxFee) + (TransferFees.totalFee - TransferFees.taxFee));
        charityWallet.transfer(toCharity);
        marketingWallet.transfer(address(this).balance);
        emit SwapToMarketing(swappedAmount);
    }
    // Swaps Token for Eth
    ...
    function setBuyFees(uint16 _liquidityFee, uint16 _marketingFee, uint16 _taxFee, uint16 _charityFee) public onlyOwner {
        require(_liquidityFee <= MaxFees.liquidityFee && _marketingFee <= MaxFees.marketingFee && _charityFee <= MaxFees.charityFee);
        BuyFees = IFees({
            liquidityFee: _liquidityFee,
            marketingFee: _marketingFee,
            taxFee: _taxFee,
            charityFee: _charityFee,
            totalFee: _liquidityFee + _marketingFee + _taxFee + _charityFee
        });
    }

    function setSellFees(uint16 _liquidityFee, uint16 _marketingFee, uint16 _taxFee, uint16 _charityFee) public onlyOwner {
        require(_liquidityFee <= MaxFees.liquidityFee && _marketingFee <= MaxFees.marketingFee && _charityFee <= MaxFees.charityFee);
        SellFees = IFees({
            liquidityFee: _liquidityFee,
            marketingFee: _marketingFee,
    ...
    function setTransferFees(uint16 _liquidityFee, uint16 _marketingFee, uint16 _taxFee, uint16 _charityFee) public onlyOwner {
        require(_liquidityFee <= MaxFees.liquidityFee && _marketingFee <= MaxFees.marketingFee && _charityFee <= MaxFees.charityFee);
        TransferFees = IFees({
            liquidityFee: _liquidityFee,
            marketingFee: _marketingFee,
    ...
    function setMaxFee(uint16 _taxFee, uint16 _liquidityFee, uint16 _marketingFee, uint16 _charityFee, bool resetFees) public onlyOwner {
        if(!tradingOpen){
            MaxFees = IFees({
                taxFee: _taxFee,
                liquidityFee: _liquidityFee,
                marketingFee: _marketingFee,
                charityFee: _charityFee,
                totalFee: _taxFee + _liquidityFee + _marketingFee + _charityFee
            });
            setBuyFees(_liquidityFee, _marketingFee, _taxFee, _charityFee);                
            setSellFees(_liquidityFee, _marketingFee, _taxFee, _charityFee);
            setTransferFees(_liquidityFee / (10), _marketingFee/ (10), _taxFee / (10), _charityFee / 10);
        }else{
            require(_liquidityFee <= MaxFees.liquidityFee && _marketingFee <= MaxFees.marketingFee && _taxFee <= MaxFees.taxFee && _charityFee <= MaxFees.charityFee);
            MaxFees = IFees({
                taxFee: _taxFee,
                liquidityFee: _liquidityFee,
                marketingFee: _marketingFee,
