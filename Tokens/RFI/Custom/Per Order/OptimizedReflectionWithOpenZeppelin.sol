...
contract Tokentest is Pausable, Ownable, ERC20 {
    ...
    mapping(address => uint) _reflectionsOwned;
    ...
    mapping(address => bool) _isReflectionExempt;
    ...
    constructor(uint startingSupply, string memory name, string memory symbol, address initialRouter) ERC20(name, symbol) {
        ...
        setTransactionLimitExempt(thisCa, true);
        setTransactionLimitExempt(_msgSender(), true);

        setMarketingFeeReceiver(_msgSender());

        setFeeExempt(thisCa, true);
        setFeeExempt(_msgSender(), true);

        setCooldownExempt(thisCa, true);
        setCooldownExempt(_msgSender(), true);
        setCooldownExempt(address(router), true);

        allowPreTrading(_msgSender(), true);
        setLiquidityHolder(_msgSender(), true);  
    ...
    // Exempt Holder From Max Wallet and Transaction Limit Functions
    ...
    function setTransactionLimitExempt(address holder, bool setOrRemove) public onlyOwner{
    ...
    function setFee(address sender, address recipient) internal {
        ...
        // removes fee if sender or recipient is fee excluded
    ...
    // In Contract Airdrop 
    function airDropTokens(address[] memory addresses, uint256[] memory amounts) external lockTheSwap {
    ...
    // Transfer Limits And Transfer Checks
    ...
    function checkCooldown(address from, address to) internal view returns (bool buyOrSell) {
    ...
    function shouldSwap(address to) internal view returns(bool) {
        return LiquiditySettings.swapEnabled
        && !LiquiditySettings.inSwap 
        && balanceOf(address(this)) >= LiquiditySettings.numTokensToSwap
        && _isLiquidityPair[to];
    }

    function liquidityOrMarketing() internal view returns(bool) {
        if(LiquiditySettings.liquidityFeeAccumulator >= LiquiditySettings.numTokensToSwap){
            return true;
        } else {
            return false;
        }
    ...
    function _transfer(address from, address to, uint256 amount) internal override whenNotPaused() {
        preTransferCheck(from, to, amount);
        if(LiquiditySettings.inSwap) { 
            ...
            if (limits(from, to)) {
                ...
                if(Launch.launched){
                    ...
                    checkCooldown(from, to) ? require(setCooldown(from, to, true)) : require(setCooldown(from, to, false)); 
                    if(block.timestamp >= LiquiditySettings.lastSwap + LiquiditySettings.swapInterval){
                        liquidityOrMarketing() ? swapAndLiquify : swapForMarketing;
                    }            
                }
            }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if(!Launch.launched || Launch.launched && !_feesEnabled){
